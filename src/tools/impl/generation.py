...
LOCATION_MAP_SYSTEM = """你是小说地点分析专家。从知识库和章节内容中提取所有地点，并分析它们之间的空间关系。
输出严格JSON: {"nodes": [{"id": "自动生成", "name": "地点名", "type": "city/building/region/world/room/other", "description": "一句话描述", "parent": "所属上级地点名（如有）"}], "connections": [{"from": "地点A", "to": "地点B", "label": "连接方式", "type": "path/portal/contains/near"}]}
...
async def _generate_location_map(loop, args: dict, kb, book_id: str, msg: str = "", queue=None) -> str:
    if queue:
        await queue.put({"_progress": "收集地点信息..."})
    entities = kb.list_entities()
    ...
    for c in outline.get("chapters", []):
        if c and c.get("synopsis"):
            outline_text += f"\n{c.get('title', '')}: {c['synopsis']}"
    if not loc_text and not outline_text:
        chapters = json_store.load_chapters(book_id)
        for ch in chapters[:10]:
            view = json_store._chapter_view(ch) if "versions" in ch else ch
            content = view.get("content", "")
            if content:
                outline_text += f"\n{view.get('title', '')}: {content[:500]}"
    if not loc_text and not outline_text:
        return "没有可分析的内容。请先导入章节或提取知识。"
    if queue:
        await queue.put({"_progress": "分析地点关系..."})
    prompt = ""
    if loc_text:
        prompt += f"## 已知地点\n{loc_text}\n\n"
    if outline_text:
        prompt += f"## 章节概要\n{outline_text[:config.storage.max_extraction_chars]}\n\n"
    prompt += "请提取所有地点并分析空间关系，输出JSON:"
    try:
        response = await asyncio.wait_for(
            loop.run_in_executor(_ai_executor, llm_chat, prompt, LOCATION_MAP_SYSTEM, 0.1, "extraction"),
            timeout=120)
    except Exception as e:
        return f"生成失败: {str(e)[:60]}"
    import uuid
...
TIMELINE_SYSTEM = """你是小说时间线分析专家。根据章节大纲，提取关键剧情事件并安排到时间轨道上。
输出严格JSON: {"tracks": [{"id": "main", "name": "主线", "color": "#22d3ee"}], "events": [{"track_id": "main", "label": "事件名", "time_label": "书内时间标记", "description": "一句话描述", "order": 1, "chapter_ref": "#1", "characters": ["角色名"]}]}
关于 time_label：用书中的叙事时间标记，如"1981年万圣节"等，不要写"第X章"。"""

async def _generate_timeline(loop, args: dict, kb, book_id: str, msg: str = "", queue=None) -> str:
    outline = json_store.get_outline(book_id)
    chapters = json_store.load_chapters(book_id)
    if not outline.get("chapters") and not chapters:
        return "需要先生成大纲或导入章节。"
    if queue:
        await queue.put({"_progress": "分析大纲，生成时间线..."})
    source_text = ""
    if outline.get("chapters"):
        for i, c in enumerate(outline["chapters"]):
            if c.get("synopsis"):
                source_text += f"\n第{i + 1}章 {c.get('title', '')}: {c['synopsis']}"
                if c.get("key_events"):
                    source_text += f" [事件: {', '.join(c['key_events'])}]"
    else:
        for i, ch in enumerate(chapters[:20]):
            view = json_store._chapter_view(ch) if "versions" in ch else ch
            content = view.get("content", "")
            if content:
                source_text += f"\n第{i + 1}章 {view.get('title', '')}: {content[:300]}"
    if not source_text:
        return "没有可分析的内容。"
    prompt = f"## 章节大纲/内容\n{source_text}\n\n请提取关键事件并安排时间线，输出JSON:"
    try:
        response = await asyncio.wait_for(
            loop.run_in_executor(_ai_executor, llm_chat, prompt, TIMELINE_SYSTEM, 0.1, "extraction"),
            timeout=120)
    except Exception as e:
        return f"生成失败: {str(e)[:60]}"
    result = _parse_progressive_result(response)
    tracks = result.get("tracks", [{"id": "main", "name": "主线", "color": "#22d3ee"}])
    events = result.get("events", [])
    import uuid
    for ev in events:
        ev["id"] = f"evt_{uuid.uuid4().hex[:8]}"
    timeline = {"tracks": tracks, "events": events}
    json_store.save_timeline(book_id, timeline)
    if queue:
        await queue.put({"_progress": f"✅ 生成完成: {len(tracks)} 条轨道, {len(events)} 个事件"})
    return f"时间线生成完成: {len(tracks)} 条轨道, {len(events)} 个事件"

def _get_timeline_tool(book_id: str) -> str:
    tl = json_store.load_timeline(book_id)
    events = tl.get("events", [])
    if not events:
        return "尚未生成时间线。请使用 generate_timeline 工具。"
