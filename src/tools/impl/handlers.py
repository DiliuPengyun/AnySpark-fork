...
def _handle_volume(name: str, args: dict, book_id: str) -> str:
    if name == "create_volume":
        ...
        story_line = args.get("story_line", "")
        if not title:
        ...
        data = {}
        # snake_case → camelCase conversion for story_line
        if "story_line" in args and args["story_line"] is not None:
            data["storyLine"] = args["story_line"]
        for key in ("title", "order"):
