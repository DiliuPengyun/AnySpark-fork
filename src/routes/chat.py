...
async def _agent_loop_sse(msg: str, req: MessageRequest, handle=None):
    ...
    sid = req.session_id or req.book_id
    if sid:
        event_store.append(sid, "user_message", {
            "text": msg,
            "mode": req.mode,
        })
    ...
    async for event in run_agent_loop(msg, agent_config, history_msgs, handle=handle):
        ...
        # ── Event sourcing: write events in real time ──
        if sid:
            if event.type == "chunk":
                event_store.append(sid, "chunk", {"text": event.data.get("text", "")})
            elif event.type == "done":
                event_store.append(sid, "done", {
    ...
    # ── Insurance: if the done event had a trivial message but no useful
    # content was produced (e.g. tool failures), inject a clear report.
    elif not final_text or final_text in ("处理出错", "完成"):
        yield {"event": "done", "data": json.dumps({
            "message": "操作已完成，但未生成有效回复。如果工具调用失败，请检查后端日志或将错误信息反馈给开发者。",
        }, ensure_ascii=False)}
...
def _load_history_as_llm_messages(session_id: str, max_turns: int = 20) -> list[dict]:
    ...
    Uses EventStore (JSONL) for new sessions, falling back to the legacy
    JSON-based message store for old sessions. EventStore provides step-level
    replay including partial chunks, while the legacy store only has final
    user+agent pairs.
    ...
    # Try EventStore first (new format)
    if event_store.has_events(session_id):
        history = event_store.replay_messages(session_id)
        if history:
...
async def _extract_shortcut(text: str, kb, book_id: str, session_id: str = "", original_msg: str = ""):
    """Extract knowledge from text via the /s shortcut.

    Robustness: the background extraction thread and the subsequent
    accept_proposal call are both wrapped so that ANY failure still
    produces a 'done' SSE event — the frontend never silently hangs.
    """
    ...
    while True:
        ...
        if evt["event"] == "result":
            ...
            try:
                ...
                logger.warning("accept_proposal failed for /s command: %s", exc)
                err_msg = f"❌ 入库失败: {str(exc)[:200]}"
                _persist_turn(book_id, session_id, original_msg, err_msg, mode="write")
                yield {"event": "done", "data": json.dumps({"message": err_msg}, ensure_ascii=False)}
            got_done = True
            break
        elif evt["event"] == "error":
            err_msg = f"❌ 提取失败: {evt['data'].get('message', '未知错误')}"
            _persist_turn(book_id, session_id, original_msg, err_msg, mode="write")
            yield {"event": "done", "data": json.dumps({"message": err_msg}, ensure_ascii=False)}
            got_done = True
            break

    # Fallback: extraction produced neither 'result' nor 'error' (e.g.
    # generator returned early, or thread died silently before the sentinel).
    if not got_done:
        fallback_msg = (
            "⚠️ 提取完成，但未收到结果数据。"
            "可能文本内容过短或格式不匹配，请提供更详细的角色设定文本。"
        )
        _persist_turn(book_id, session_id, original_msg, fallback_msg, mode="write")
        yield {"event": "done", "data": json.dumps({"message": fallback_msg}, ensure_ascii=False)}
