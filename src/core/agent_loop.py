    ...
    try:
        while not state.exhausted:
            ...
            if not response.tool_calls:
                ...
                if _looks_like_plan_without_action(final_text) and state.plan_nudges < 2:
                    state.plan_nudges += 1
        ...
        # hard round cap was hit. Give a deterministic terminal message.
        if not state.metrics.finish_reason:
            state.metrics.finish_reason = "round_limit_reached"
            limit_msg = (
                f"已达循环轮次上限（{state.max_rounds} 轮），本轮停止。共执行 {state.metrics.tool_calls} 次工具调用。"
            )
            yield LoopEvent(type="text", data={"content": limit_msg})
            yield LoopEvent(
                type="done",
                data={
                    "message": limit_msg,
...
def _prepare_initial_messages(
    ...
    messages.append({"role": "user", "content": mode_marker + user_message})
    return messages
...
async def _stream_llm_response(messages, tools, agent_config: AgentConfig, handle, temperature: float = None):
...
def _sanitize_tool_messages(messages: list[dict]) -> None:
    ...
    # Step 3: any call ids with no response yet get placeholders. Append at
    # the tail — the API accepts tool messages in any position as long as
    # every id is answered somewhere in the conversation.
    for tc_id in all_call_ids - answered:
        messages.append(
            {
                "role": "tool",
                "tool_call_id": tc_id,
