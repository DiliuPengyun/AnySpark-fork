...
registry.register(Tool(
    ...
    description="【定稿】一键闭环：章节状态 draft → final + 验证 + 增量提取知识 + 伏笔检查。仅当章节最终定稿时使用，草稿阶段不要调用。如需草稿阶段单独提取知识请用 extract_chapter。",
    parameters={"chapter_id": {"type": "string", "description": "章节序号，如 #5 或 #E1（番外）"}},
))
...
# ── Foreshadow lifecycle scheduling tools ──
...
registry.register(Tool(
    ...
    parameters={
        ...
        "auto_extract": {"type": "boolean", "description": "是否每章写完自动提取知识（不推荐，定稿后用 finalize_chapter 即可），默认 false", "required": False},
    },
    dangerous=True,
))
