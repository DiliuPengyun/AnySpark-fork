...
如果没有任何段落回收了该伏笔，输出 {{"found": false}}"""
    ...
    def check_consistency(self) -> dict:
        ...
        # 4b. Spatial containment cycle: A LOCATED_IN B and B LOCATED_IN A
        cycle_rows = self._run("""
        ...
        for r in cycle_rows:
            issues.append({
                "type": "spatial_cycle",
                "severity": "high",
                "description": f"空间包含循环: 「{r['name_a']}」位于「{r['name_b']}」内，但「{r['name_b']}」也位于「{r['name_a']}」内",
            })

        # 4c. Isolated location: location with no spatial relationships
        iso_loc_rows = self._run("""
        ...
        """, {"pid": self.project_id})
        for r in iso_loc_rows:
            issues.append({
                "type": "isolated_location",
                "severity": "medium",
                "description": f"地点「{r['name']}」无任何空间关系连接，可能是空间关系提取遗漏",
            })
    ...
    def _compute_graph_insights(self) -> dict:
        ...
        if timeline_events:
            ...
            insights["forgotten_character_count"] = len(important_forgotten)
        ...
        # 2. Unresolved foreshadows — use same logic as ForeshadowBoard
        ...
        if open_fores:
            ...
            if len(open_fores) > 3:
                insights["suggestions"].append({
                    "type": "reminder",
                    "priority": "medium",
                    "message": f"有 {len(open_fores)} 个伏笔尚未回收，注意适时推进伏笔线。"
                })
        ...
        # 5. Underutilized locations — check ALL relationship edges, not just INVOLVES
        locs = self.list_entities(entity_type="location")
        if locs:
            loc_usage = {}
            for loc in locs:
                cnt = self._run_single("""
                    MATCH (l:Entity:Location {id: $eid, project_id: $pid})-[r]-(other:Entity {project_id: $pid})
                    RETURN count(r) as cnt
                """, {"eid": loc.id, "pid": self.project_id})
                loc_usage[loc.name] = cnt["cnt"] if cnt else 0
            ...
            if unused:
                ...
                insights["suggestions"].append({
                    "type": "info",
                    "priority": "low",
                    "message": f"有 {len(unused)} 个地点未关联任何实体或事件：{', '.join(unused[:3])}。"
                })
    ...
    # ── P3-ext: Narrative diagnosis — intelligent reasoning engine ──

    def get_narrative_diagnosis(self) -> dict:
        """Generate a structured diagnosis report with causal reasoning chains."""
    ...
    def _compute_narrative_diagnosis(self) -> dict:
        ...
        # ── Dimension scoring (non-linear, weighted) ──
        forgotten = raw.get("forgotten_characters", [])
        forgotten_count = raw.get("forgotten_character_count", len(forgotten))
        foreshadows = raw.get("unresolved_foreshadows", [])
        unresolved_fs_count = raw.get("unresolved_foreshadow_count", len(foreshadows))
        disconnected = raw.get("disconnected_pairs", [])
        disconnected_count = raw.get("disconnected_pair_count", len(disconnected))
        bridges = raw.get("bridge_characters", [])
        bridge_count = raw.get("bridge_character_count", len(bridges))
        unused_locs = raw.get("underutilized_locations", [])
        unused_loc_count = raw.get("unused_location_count", len(unused_locs))
        ...
        total_fores = len(self.list_foreshadows())  # includes resolved

        # Dimension 1: Character continuity
        # Uses exponential decay with importance weighting
        if forgotten_count > 0:
            important = sum(1 for c in forgotten if c.get("important"))
            weighted = important * 2 + (forgotten_count - important)
            max_weighted = max(total_chars, 1) * 2
            rate = weighted / max_weighted
            char_score = round(100 * (1 - rate ** 0.4))
        else:
            char_score = 100
        char_finding = (
            f"{forgotten_count} 个重要角色多章未出场" if forgotten_count > 0
            else "所有角色出场连贯，无遗忘风险"
        )

        # Dimension 2: Foreshadow management
        # Uses logarithmic decay — first few unresolved are cheap, many are expensive
        import math
        if unresolved_fs_count > 0:
            unresolved_rate = unresolved_fs_count / max(total_fores, 1)
            fore_score = round(100 * (1 / (1 + math.log(1 + unresolved_rate * 8))))
        else:
            fore_score = 100
        fore_finding = (
            f"{unresolved_fs_count} 个伏笔待回收" if unresolved_fs_count > 0
            else "所有伏笔已回收，叙事闭合良好"
        )

        # Dimension 3: Relationship network
        # Density-normalized: high-density graphs suffer more from disconnection
        if total_chars > 2:
            max_pairs = total_chars * (total_chars - 1) / 2
            disc_rate = min(disconnected_count / max(max_pairs, 1), 1.0)
            # Density penalty factor
            metrics = self._compute_worldbuilding_metrics()
            density = metrics.get("density", 0)
            density_factor = 1 + (1 - min(density * 10, 1))  # low density → higher penalty
            rel_score = round(100 * (1 - disc_rate * 0.7 * density_factor))
        else:
            rel_score = 100
        rel_finding = (
            f"{disconnected_count} 对角色之间无关联路径" if disconnected_count > 0
            else "角色关系网络连接良好"
        )
        ...
        if unused_loc_count > 0 and total_locs > 0:
            loc_rate = unused_loc_count / total_locs
            loc_score = round(100 * (1 - loc_rate ** 0.6))
        else:
            loc_score = 100
        loc_finding = (
            f"{unused_loc_count} 个地点从未使用" if unused_loc_count > 0
            else "所有地点的使用率合理"
        )
        ...
        if confidence:
            ...
            # Weighted: average confidence + penalty for low-confidence entities
            low_penalty = (low_conf / max(len(confidence), 1)) ** 0.5 * 30
            conf_score = round(max(0, min(100, avg_conf * 100 - low_penalty)))
            conf_finding = (
                f"{low_conf} 个实体设定可信度偏低" if low_conf
                else "所有实体设定可信度良好"
            )
        else:
            conf_score = 100
            conf_finding = "暂无设定可信度数据"

        # Dimension 6: Constraint compliance
        hard_violations = [c for c in constraints if c.get("severity") == "hard"]
        if constraints:
            hard_weight = len(hard_violations) * 35
            soft_weight = (len(constraints) - len(hard_violations)) * 12
            const_score = round(max(0, 100 * (1 - (hard_weight + soft_weight) / 100)))
        else:
            const_score = 100
        const_finding = (
            f"{len(constraints)} 条约束违反（{len(hard_violations)} 条硬约束）" if constraints
            else "所有叙事约束均已满足"
        )

        # Weighted dimensions by importance
        dimensions = [
            {"name": "角色连贯性", "score": char_score, "finding": char_finding, "icon": "users", "weight": 0.20},
            {"name": "伏笔管理", "score": fore_score, "finding": fore_finding, "icon": "target", "weight": 0.15},
            {"name": "关系网络", "score": rel_score, "finding": rel_finding, "icon": "git-merge", "weight": 0.15},
            {"name": "地点利用", "score": loc_score, "finding": loc_finding, "icon": "map-pin", "weight": 0.10},
            {"name": "设定可信度", "score": conf_score, "finding": conf_finding, "icon": "microscope", "weight": 0.15},
            {"name": "约束合规", "score": const_score, "finding": const_finding, "icon": "shield", "weight": 0.25},
        ]
        ...
        # Chain 1: Forgotten characters → foreshadow risk
        if forgotten and foreshadows:
            forgotten_names = [c["name"] for c in forgotten[:3]]
            chains.append({
                "cause": f"重要角色 {', '.join(forgotten_names)} 多章未出场",
                "effect": f"{len(foreshadows)} 个伏笔可能无法回收",
                "link": "伏笔的回收通常需要相关角色出场来触发",
                "severity": "high",
                "suggestion": f"优先安排 {forgotten_names[0]} 在下一章出场，并规划回收相关伏笔",
            })

        # Chain 2: Disconnected pairs → fragmentation risk
        if disconnected and bridges:
            bridge_name = bridges[0]["entity_name"]
            chains.append({
                "cause": f"{len(disconnected)} 对角色之间无关联",
                "effect": "可能导致故事线分裂，读者难以跟随",
                "link": f"枢纽角色 {bridge_name} 可作为桥梁连接断裂的关系",
                "severity": "medium" if len(disconnected) > 3 else "low",
                "suggestion": f"考虑通过 {bridge_name} 的视角或事件间接连接这些角色",
            })

        # Chain 3: Low confidence → characterization risk
        if confidence:
            low_conf = [c for c in confidence if c["confidence"] < 0.3]
            if low_conf and forgotten:
                chains.append({
                    "cause": f"{len(low_conf)} 个实体设定可信度低",
                    "effect": "角色形象可能单薄，缺乏深度",
                    "link": "设定可信度低的实体往往也是出场次数少的实体",
                    "severity": "medium",
                    "suggestion": f"增加 {low_conf[0]['entity_name']} 的出场场景，丰富其设定细节",
                })

        # Chain 4: Constraint violations → continuity risk
        if hard_violations:
            chains.append({
                "cause": f"{len(hard_violations)} 条硬约束被违反",
                "effect": "可能导致叙事逻辑矛盾，读者产生困惑",
                "link": "硬约束通常定义了角色的核心特征或世界观基本规则",
                "severity": "high",
                "suggestion": f"优先修复硬约束违反：{hard_violations[0].get('description', '')[:80]}",
            })

        # Chain 5: Unused locations → missed opportunities
        if unused_locs and foreshadows:
            chains.append({
                "cause": f"{len(unused_locs)} 个地点从未使用",
                "effect": "可能错过回收伏笔的场景机会",
                "link": "未使用的地点可以作为伏笔回收的场景",
                "severity": "low",
                "suggestion": f"考虑将地点 {unused_locs[0]} 作为某个伏笔回收的场景",
            })
        ...
        if overall >= 90:
            ...
            weak_dims = [d for d in dimensions if d["score"] < 70]
            summary = f"整体良好，但 {len(weak_dims)} 个维度需要关注：{'、'.join(d['name'] for d in weak_dims[:2])}。"
        elif overall >= 50:
            summary = f"需关注 {len([d for d in dimensions if d['score'] < 60])} 个维度，建议优先处理高优先级推理链。"
        else:
            summary = "叙事结构存在较多问题，建议系统性地修复约束违反和角色遗忘问题。"

        # ── Action items (sorted by severity) ──
        action_items = []

        # Add constraint fixes (highest priority)
        ...
        # Add forgotten character suggestions
        if forgotten:
            action_items.append({
                "priority": "high",
                "action": f"安排 {forgotten[0]['name']} 出场或提及",
                "category": "角色连贯性",
            })

        # Add foreshadow resolution
        if foreshadows:
            action_items.append({
                "priority": "medium",
                "action": f"规划回收 {len(foreshadows)} 个待回收伏笔",
                "category": "伏笔管理",
            })

        # Add bridge character suggestions
        if bridges and disconnected:
            action_items.append({
                "priority": "medium",
    ...
    def _compute_character_importance(self) -> list[dict]:
        ...
        max_appear = max(appear_map.values()) if appear_map else 1

        results = []
        for r in degree_rows:
            cid = r["id"]
            deg = r["degree"]
            appear = appear_map.get(cid, 0)
...
- 每条建议必须给出简短理由"""
    ...
    def _compute_location_importance(self) -> list[dict]:
        ...
        max_events = max(event_map.values()) if event_map else 1
    ...
    def _compute_organization_importance(self) -> list[dict]:
        ...
        max_degree = max((r["degree"] for r in degree_rows), default=1)
        max_members = max(member_map.values()) if member_map else 1
        max_events = max(event_map.values()) if event_map else 1
