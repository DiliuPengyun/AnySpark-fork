...
> ⚠️ **先行版（Pre-release）声明**：当前版本为先行体验版，部分功能可能不完善或存在 bug。我们正在积极收集体验者反馈并持续修复。如遇到问题，欢迎通过 GitHub Issues 或 QQ 群反馈。感谢你的理解与支持！
...
| 🧠 自主决策 | 🎯 知识约束 | 🔄 持久运行 |
|:---:|:---:|:---:|
| Agent 自主规划执行多步创作任务，你只需给出方向，它来完成全流程 | Neo4j 知识图谱确保角色、世界观、事件严格一致，告别前后矛盾 | Autopilot 引擎支持后台持久任务，一句话指令即可驱动整本书的批量创作 |

| 🎨 全流程覆盖 | 🛡️ 工程级可靠 | 🔌 高度可扩展 |
|:---:|:---:|:---:|
| 从灵感构思 → 大纲规划 → 章节写作 → 评审修订 → 全书变换，一套工具走完全程 | 幻觉安全网（fake_tool/fake_write 两层 warning-only）、Doom Loop 防护、Book 级写锁、指数退避重试——生产级 Agent 基础设施 | 插件系统、技能 YAML、文风 Profiles、自定义评审员——所有创作范式均可定制 |
...
3. **角色体系** — 自动抽取并管理角色档案，Neo4j 图谱确保人物关系始终一致
4. **逐章写作** — 每章写作前自动检索当前时空点的所有已知事实，杜绝前后矛盾
5. **评审迭代** — 8 位评审员多维度审视每章质量，不满意随时调方向
...
| 功能 | 说明 |
|------|------|
| **Neo4j 知识图谱** | 角色、世界观、事件、关系的结构化存储，Cypher 图查询 + 自动知识抽取 |
...
| 功能 | 说明 |
|------|------|
| **评审团系统** | 8 位预设评审员（编剧/文学编辑/逻辑审校/爽文读者/情感型读者/硬核党/挑刺王/休闲读者），支持并发/串行评审 |
| **自定义评审员** | 通过 YAML 人设文件自定义评审员的人格、关注点和评价标准 |
| **质量门控** | 复用评审团轻量评审，章节质量低于阈值自动暂停，确保输出水准 |
| **幻觉安全网** | fake_tool/fake_write 两层检测（warning-only，不中断执行），遵循「结构化验证优先、文本检测兜底」原则 |
| **一致性校验** | 知识冲突自动检测，写作内容与知识库事实严格对齐 |
| **剧情链系统** | 章节级情节节拍提取与对比，支持 plot chain 结构化管理与 diff 审查 |
...
| **文档导入** | 支持 txt / docx / md 文件上传解析，自动提取知识与章节结构 |
...
| 你需要什么 | 说明 | 如何获取 |
|-----------|------|---------|
| **Docker Desktop** | 唯一必须安装的软件 | [Windows](https://docs.docker.com/desktop/install/windows-install/) / [Mac](https://docs.docker.com/desktop/install/mac-install/) / [Linux](https://docs.docker.com/desktop/install/linux-install/) |
| **DeepSeek API Key** | 用于调用 AI 大模型 | [platform.deepseek.com](https://platform.deepseek.com) 注册后获取（新用户有免费额度） |
| **网络连接** | 拉取镜像需要 | 首次部署约需下载 1~2GB |

> 💡 **为什么需要 Docker？** 火花依赖 Neo4j 图数据库来管理角色、世界观和时间线，确保你的故事前后一致。Neo4j 需要独立的 Java 运行时环境，Docker 把它打包好了，让你一键启动，无需自己安装 Java、配置数据库。**Docker 是唯一需要安装的软件**——Python、Node.js 等全部由 Docker 镜像内置，无需手动安装。
...
# 4. 打开 http://localhost:8190
```

> Mac/Linux 如果 pygit2 安装失败：`brew install libgit2`（Mac）或 `apt install libgit2-dev`（Ubuntu）
...
<details>
<summary><b>为什么需要 Docker？能不用吗？</b></summary>

火花的核心是 Neo4j 图数据库，它负责管理角色关系、世界观一致性、时间线等复杂知识。Neo4j 需要 Java 运行时，Docker 把它打包好了，你只需安装 Docker 一个软件。

如果不用 Docker，你需要手动安装：Java 运行时 + Neo4j 数据库 + Python 3.11+ + Node.js 20+，并分别配置启动——比装 Docker 复杂得多。

**如果你实在不想装 Docker**，可以访问在线体验站：[www.anyspark.xyz](https://www.anyspark.xyz)（⚠️ 限人数，建议自部署）。
</details>
...
| `NEO4J_PASSWORD` | Neo4j 密码 | `novel_agent_2024!` |
...
| [MODULES.md](MODULES.md) | 16 个核心模块定义、接口与实现状态 |
| [TECH_STACK.md](TECH_STACK.md) | 完整技术栈与版本 |
| [ROADMAP.md](ROADMAP.md) | 开发路线图与版本历史 (v1.0 → v3.0) |
| [IMPROVEMENTS.md](IMPROVEMENTS.md) | 改进跟踪 (18/18 已完成) |
