...
const NODE_TYPE_LABELS: Record<string, string> = {
  character: '角色', location: '地点', organization: '组织', event: '事件', item: '物品', skill: '技能', race: '种族', concept: '概念', unknown: '未知',
}

const NODE_STYLES = {
  character:     { color: DATA_COLORS.character.fill, stroke: DATA_COLORS.character.stroke, icon: 'user' },
  location:      { color: DATA_COLORS.location.fill, stroke: DATA_COLORS.location.stroke, icon: 'map-pin' },
  organization:  { color: DATA_COLORS.organization.fill, stroke: DATA_COLORS.organization.stroke, icon: 'building' },
  event:         { color: DATA_COLORS.event.fill, stroke: DATA_COLORS.event.stroke, icon: 'zap' },
  item:          { color: DATA_COLORS.item.fill, stroke: DATA_COLORS.item.stroke, icon: 'star' },
  skill:         { color: DATA_COLORS.skill.fill, stroke: DATA_COLORS.skill.stroke, icon: 'zap' },
  race:          { color: DATA_COLORS.race.fill, stroke: DATA_COLORS.race.stroke, icon: 'users' },
  concept:       { color: DATA_COLORS.concept.fill, stroke: DATA_COLORS.concept.stroke, icon: 'lightbulb' },
  unknown:       { color: DATA_COLORS.unknown.fill, stroke: DATA_COLORS.unknown.stroke, icon: 'info' },
}

export default function RelationGraph({ bookId, characters, timelineEvents, selectedTimeOrder, onSelectChar }) {
  const svgRef = useRef(null)
  const containerRef = useRef(null)
  const dimensions = useResizeObserver(containerRef)
  const [error, setError] = useState(null)
  const simulationRef = useRef(null)
  // P1-2: Multi-hop graph mode — activates get_entity_network API
  const [multiHopMode, setMultiHopMode] = useState(false)
  const [expandedNodes, setExpandedNodes] = useState<Set<string>>(new Set())
  const [extraNodes, setExtraNodes] = useState<any[]>([])
  const [extraLinks, setExtraLinks] = useState<any[]>([])

  async function fetchEntityNetwork(entityId: string) {
    try {
      const res = await fetch(`/api/books/${bookId}/graph/${entityId}?depth=2`)
      if (!res.ok) return
      const data = await res.json()
      const newNodes = (data.nodes || []).filter((n: any) => !expandedNodes.has(n.id) && !characters.find((c: any) => c.id === n.id))
      const newLinks = (data.edges || []).map((e: any) => ({
        source: e.from, target: e.to, type: (e.type || '').toLowerCase(), timePoint: '',
      }))
      setExpandedNodes((prev: Set<string>) => new Set([...prev, entityId]))
      setExtraNodes((prev: any[]) => [...prev, ...newNodes])
      setExtraLinks((prev: any[]) => [...prev, ...newLinks])
    } catch (e) {
      console.error('Entity network fetch failed:', e)
    }
  }
  ...
  }, [characters, selectedTimeOrder, dimensions, extraNodes, extraLinks])
  ...
  function renderGraph() {
    ...
    const selOrder = selectedTimeOrder > 0 ? selectedTimeOrder : Infinity
    ...
    // P1-2: Merge multi-hop data from get_entity_network API
    if (extraNodes.length > 0) {
      for (const n of extraNodes) {
        if (!nodeIds.has(n.id)) {
          nodes.push({
            id: n.id, name: n.name || n.id.slice(0, 8),
            relationCount: 0, nodeType: n.type || 'unknown',
          })
          nodeIds.add(n.id)
        }
      }
    ...
    const node = g.append('g').attr('class', 'nodes').selectAll('circle').data(visibleNodes).join('circle')
      ...
      .on('click', (event, d) => {
        ...
        // P1-2: In multi-hop mode, expand the clicked node's network
        if (multiHopMode && !expandedNodes.has(d.id)) {
          fetchEntityNetwork(d.id)
        }
      })
    ...
    const label = g.append('g').attr('class', 'labels').selectAll('text').data(visibleNodes).join('text')
      .text(d => {
        ...
        const label = NODE_TYPE_LABELS[d.nodeType] || '?'
        return `[${label}]${d.name}`
      })
  ...
  if (error) return <div className="flex-1 flex items-center justify-center text-red-400 text-sm gap-1.5"><Icon name="alert-triangle" size={14} /> {error}</div>
  if (!characters || characters.length === 0)
    return <div className="flex-1 flex items-center justify-center text-zinc-500 text-sm">无角色数据</div>

  return (
    <div className="flex-1 flex flex-col relative overflow-hidden">
      <div className="px-4 py-2 border-b border-zinc-800 flex gap-4 flex-wrap text-[10px] text-zinc-500 items-center">
        <button
          onClick={() => {
            setMultiHopMode(!multiHopMode)
            if (!multiHopMode) {
              setExtraNodes([])
              setExtraLinks([])
          ...
          className={`px-2 py-0.5 rounded text-[10px] border ${multiHopMode ? 'bg-blue-600 text-white border-blue-500' : 'bg-zinc-800 text-zinc-400 border-zinc-700'}`}
        >
          {multiHopMode ? '✓ 多跳展开' : <><Icon name="search" size={10} /> 多跳展开</>}
        </button>
      ...
      <div className="px-4 py-1.5 border-t border-zinc-800 text-[10px] text-zinc-600">
        拖拽节点移动 · 滚轮缩放 · 点击角色查看详情{multiHopMode ? ' · 点击节点展开多跳关系网络' : ''}
      </div>
    </div>
  )
}
