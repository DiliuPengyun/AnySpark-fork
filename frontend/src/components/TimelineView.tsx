...
export default function TimelineView({ bookId }) {
  ...
  // 4D Map: Event entity details
  const [eventEntities, setEventEntities] = useState<any[]>([])
      ...
      trackEvents.forEach((ev, ei) => {
        ...
        g.append('circle')
          ...
          .on('mouseenter', function (event) {
            ...
            if (ev.characters?.length) lines.push(`[角色] ${ev.characters.join(', ')}`)
        ...
        {selected && (
          <div className="w-56 border-l border-zinc-800 bg-zinc-900/80 p-4 shrink-0 overflow-y-auto">
            ...
            <div className="space-y-1.5 text-[11px]">
              ...
              {/* 4D Map: Affected entities */}
              {eventEntities.length > 0 && (
                <div className="mt-3 pt-3 border-t border-zinc-800">
                  <p className="text-[10px] text-blue-400 font-medium mb-1.5 flex items-center gap-1"><Icon name="clock" size={10} /> 关联实体</p>
                  {eventEntities.map((name: string, i: number) => (
                    <p key={i} className="text-[10px] text-zinc-400"><Icon name="user" size={10} className="inline mr-0.5" />{name}</p>
                  ))}
                </div>
              )}
              <div className="mt-3 pt-3 border-t border-zinc-800">
                <p className="text-[10px] text-zinc-600">
                  已同步时间轴到 #{selected.order || selected.time_order || '?'}
                </p>
              </div>
            </div>
          </div>
