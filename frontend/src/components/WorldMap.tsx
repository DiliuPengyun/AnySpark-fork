...
export default function WorldMap({ bookId }) {
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      {/* 4D Map: Time-aware character locations */}
      {timeMap && Object.keys(timeMap.characters_at_locations || {}).length > 0 && (
        <div className="px-4 py-2 border-t border-zinc-800 bg-zinc-900/50">
          <div className="text-[10px] text-blue-400 font-medium mb-1.5 flex items-center gap-1">
            <Icon name="clock" size={10} /> #{selectedTimeOrder} · 角色位置
          </div>
          <div className="flex flex-wrap gap-1.5">
            {Object.entries(timeMap.characters_at_locations).map(([locId, locData]: [string, any]) => (
              <span key={locId} className="text-[10px] bg-zinc-800 text-zinc-400 px-2 py-0.5 rounded">
                <Icon name="map-pin" size={10} className="inline mr-0.5" />{locData.location_name}
                {locData.characters?.map((c: any) => (
                  <span key={c.id} className="text-zinc-300 ml-1"><Icon name="user" size={10} className="inline mr-0.5" />{c.name}</span>
                ))}
              </span>
            ))}
          </div>
