...
export default function CharacterDetail({ character, timelineEvents, bookId, onClose, onUpdated }: { character: Record<string, any>; timelineEvents: any[]; bookId: string; onClose: () => void; onUpdated?: () => void }) {
  ...
  // 4D Map: Time-aware entity state
  const selectedTimeOrder = useSelectedTimeOrder()
  const [timeState, setTimeState] = useState<any>(null)

  useEffect(() => {
    if (character.id && character.type === 'character' && selectedTimeOrder >= 0) {
      fetchTimeState()
    }
  }, [character.id, selectedTimeOrder])

  async function fetchTimeState() {
    try {
      const res = await fetch(`/api/books/${bookId}/graph/state/${character.id}?time_order=${selectedTimeOrder}`)
      if (res.ok) setTimeState(await res.json())
    } catch (e) { /* silent */ }
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-[10vh]">
      ...
      <div className="relative bg-zinc-900 border border-zinc-700 rounded-2xl w-full max-w-3xl max-h-[82vh] overflow-y-auto shadow-2xl">
        <button onClick={onClose} className="absolute top-4 right-4 text-zinc-500 hover:text-zinc-300 text-lg z-10" aria-label="关闭"><Icon name="x" size={18} /></button>
        ...
        <div className="p-6">
          ...
          {/* 4D Map: Time-aware state panel */}
          {timeState && (
            <div className="mb-4 p-3 bg-blue-950/30 border border-blue-900/50 rounded-lg">
              <div className="flex items-center gap-2 mb-2">
                <span className="text-[10px] text-blue-400 font-medium flex items-center gap-1"><Icon name="clock" size={10} /> 时间点 #{selectedTimeOrder} 状态</span>
                {timeState.phase && (
                  <span className="text-[10px] bg-blue-900/50 text-blue-300 px-1.5 py-0.5 rounded">
                    阶段: {timeState.phase.phase || timeState.phase.label}
                  </span>
                )}
              </div>
              <div className="grid grid-cols-2 gap-2 text-xs">
                {timeState.location && (
                  <div className="text-zinc-400 flex items-center gap-1">
                    <Icon name="map-pin" size={10} /> <span className="text-zinc-300">{timeState.location.name}</span>
                  </div>
                )}
                <div className="text-zinc-400 flex items-center gap-1">
                  <Icon name="users" size={10} /> <span className="text-zinc-300">{timeState.relationships?.length || 0} 条已知关系</span>
                </div>
                <div className="text-zinc-400 flex items-center gap-1">
                  <Icon name="calendar" size={10} /> <span className="text-zinc-300">{timeState.events?.length || 0} 个事件</span>
                </div>
                <div className="text-zinc-400 flex items-center gap-1">
                  <Icon name="target" size={10} /> <span className="text-zinc-300">{timeState.active_foreshadows?.length || 0} 个活跃伏笔</span>
                </div>
              </div>
              {timeState.relationships?.length > 0 && (
                <div className="mt-2 flex flex-wrap gap-1">
                  {timeState.relationships.slice(0, 5).map((r: any, i: number) => (
                    <span key={i} className="text-[10px] bg-zinc-800 text-zinc-400 px-1.5 py-0.5 rounded">
                      {r.name} [{r.relationship}]
                    </span>
                  ))}
                  {timeState.relationships.length > 5 && (
                    <span className="text-[10px] text-zinc-600">+{timeState.relationships.length - 5}</span>
                  )}
                </div>
              )}
            </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <span onClick={(e) => { e.stopPropagation(); handleDeleteSnapshot(snap.id) }}
                        className="ml-2 text-zinc-500 hover:text-red-400 inline-flex"><Icon name="x" size={10} /></span>
                    </button>
                  )
                })}
              </div>
